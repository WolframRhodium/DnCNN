function lgraph = resnet_v2(patchSize, channels, networkDepth)
% ResNet_v2 for denoising
% The depth of the network should be even
% Reference:
% [1] He, K., Zhang, X., Ren, S., & Sun, J. (2016). 
%   Deep residual learning for image recognition.
%   In Proceedings of the IEEE conference on computer vision and pattern recognition (pp. 770-778).
% [2] He, K., Zhang, X., Ren, S., & Sun, J. (2016, October).
%   Identity mappings in deep residual networks. 
%    In European Conference on Computer Vision (pp. 630-645). Springer International Publishing.
    ...
    for i = 2:2:networkDepth-2
        ...
        if i == 2
            lgraph = connectLayers(lgraph, 'ReLU1', 'BNorm2');
        else
            lgraph = connectLayers(lgraph, ['Add', num2str(i-1)], ['BNorm', num2str(i)]);
        end
        lgraph = connectLayers(lgraph, ['BNorm', num2str(i)], ['ReLU', num2str(i)]);
        lgraph = connectLayers(lgraph, ['ReLU', num2str(i)], ['Conv', num2str(i)]);
        ...
        lgraph = connectLayers(lgraph, ['Conv', num2str(i)], ['BNorm', num2str(i+1)]);
        lgraph = connectLayers(lgraph, ['BNorm', num2str(i+1)], ['ReLU', num2str(i+1)]);
        lgraph = connectLayers(lgraph, ['ReLU', num2str(i+1)], ['Conv', num2str(i+1)]);
        ...
        lgraph = connectLayers(lgraph, ['Conv', num2str(i+1)], ['Add', num2str(i+1), '/in2']);
    end
